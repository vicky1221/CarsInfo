...
- (id)initWithAppKey:(NSString *)_appKey appSecret:(NSString *)_appSecrect
   ...
   ssoCallbackScheme:(NSString *)_ssoCallbackScheme
         andDelegate:(id<SinaWeiboDelegate, SinaWeiboRequestDelegate>)_delegate
{
    if ((self = [super init]))
    {
        self.appKey = _appKey;
...
- (void)request:(SinaWeiboRequest *)_request didFailWithError:(NSError *)error
{
    if (_request == request)
    {
		...
		{
			[delegate request:_request didFailWithError:error];
		}
	}
}
...
- (void)request:(SinaWeiboRequest *)_request didFinishLoadingWithResult:(id)result
{
    if (_request == request)
    {
		...
		{
			[delegate request:_request didFinishLoadingWithResult:result];
		}
	}
}
...
- (void)getUserInfo
{
	...
	[params setObject:(self.userID ? self.userID : @"") forKey:@"uid"];
...
- (void)sendWeiBoWithText:(NSString *)text imageUrl:(NSString *)imageUrl
{
    ...
    if (imageUrl)
    {
        ...
        [self requestWithURL:@"statuses/upload_url_text.json" params:params httpMethod:@"POST" delegate:delegate];
    }
    else
    {
        [self requestWithURL:@"statuses/update.json" params:params httpMethod:@"POST" delegate:delegate];
    }
}

- (void)followOfficial:(NSString *)uid
{
	NSMutableDictionary *params = [NSMutableDictionary dictionaryWithCapacity:2];
    [params setObject:(uid ? uid : @"2820905177") forKey:@"uid"];
    [self requestWithURL:@"friendships/create.json" params:params httpMethod:@"POST" delegate:delegate];
}
