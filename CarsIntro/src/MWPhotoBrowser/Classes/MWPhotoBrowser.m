...
//        UIBarButtonItem *doneButton = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Done", nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)] autorelease];
        ...
        if ([UIBarButtonItem respondsToSelector:@selector(appearance)]) {
            [doneButton setBackgroundImage:[UIImage	imageNamed:@""] forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
            [doneButton setBackgroundImage:[UIImage	imageNamed:@""] forState:UIControlStateNormal barMetrics:UIBarMetricsLandscapePhone];
            [doneButton setBackgroundImage:[UIImage	imageNamed:@""] forState:UIControlStateHighlighted barMetrics:UIBarMetricsDefault];
            [doneButton setBackgroundImage:[UIImage	imageNamed:@""] forState:UIControlStateHighlighted barMetrics:UIBarMetricsLandscapePhone];
...
- (void)reloadData {
	...
	// 重新计算_currentPageIndex
	if (_currentPageIndex == numberOfPhotos && _currentPageIndex > 0) {
		_currentPageIndex = numberOfPhotos - 1;
	}
            ...
            if ([MFMailComposeViewController canSendMail]) {
				if (isMe) {
					self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
															...
															otherButtonTitles:NSLocalizedString(@"Save", nil), NSLocalizedString(@"Copy", nil), NSLocalizedString(@"Email", nil), NSLocalizedString(@"Remove", nil), nil] autorelease];
				} else {
					self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
															cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:nil
				...
				if (isMe) {
					self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
															cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:nil
															otherButtonTitles:NSLocalizedString(@"Save", nil), NSLocalizedString(@"Copy", nil), NSLocalizedString(@"Remove", nil), nil] autorelease];
				} else {
					self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
															cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:nil
...
- (void)removePhoto {
	[[NSNotificationCenter defaultCenter] postNotificationName:@"RemovePhoto" object:[NSNumber numberWithInt:_currentPageIndex]];
}
