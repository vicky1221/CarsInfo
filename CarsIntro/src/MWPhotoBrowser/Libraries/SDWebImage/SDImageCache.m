...
static natural_t maxMemSize = 1024*1024*5;
static natural_t usedMem = 0;
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    if (image)
    {
		...
		usedMem += [self getMemorySizeOfImage:image];
...
//        size += [UIImageJPEGRepresentation(img, 0) length]; // ?? plj
		size += CGImageGetBytesPerRow(img.CGImage) * CGImageGetHeight(img.CGImage); // plj
    };
    
    return size;
}

- (int)getMemorySizeOfImage:(UIImage *)image
{
	if (image == nil)
		return 0;
